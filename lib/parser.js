...
function tokenize(template) {
  ...
  const regex = /\{(\/?block:[A-Za-z0-9_:\-]+|text:[A-Za-z0-9_:\- ]+|select:[A-Za-z0-9_:\- ]+|lang:[A-Za-z0-9_:\- ]+|[A-Za-z0-9_:\-]+)\}/g;
...
function evaluateAST(node, context) {
  ...
  if (node.type === 'block') {
    ...
    if (name.startsWith('If')) {
      const varName = name.substring(2);
      const hasValue = blog[`text:${varName}`] || blog[`if:${varName}`] === '1' || blog[`if:${varName}`] === true;
      return hasValue ? renderChildren(node, context) : '';
    }
    if (name.startsWith('IfNot')) {
      const varName = name.substring(5);
      const hasValue = blog[`text:${varName}`] || blog[`if:${varName}`] === '1' || blog[`if:${varName}`] === true;
      return !hasValue ? renderChildren(node, context) : '';
    }
